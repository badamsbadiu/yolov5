...
class BackboneWithAttention(nn.Module):
    """Backbone with integrated Spatial Attention."""

    def __init__(self, in_channels, out_channels):
        super(BackboneWithAttention, self).__init__()
        self.conv1 = Conv(in_channels, out_channels, 3, 1)  # Example layer
...
class AtrousSPPF(nn.Module):
    """Atrous Spatial Pyramid Pooling with parallel dilated convolutions."""

    def __init__(self, c1, c2, k=5):  # k is unused here but kept for compatibility
        super().__init__()
        c_ = c1 // 2  # Hidden channels
        self.conv1 = Conv(c1, c_, 1, 1)  # Initial 1x1 convolution
        self.atrous1 = Conv(c_, c_, 3, 1, p=1, d=1)  # Dilated conv with dilation rate 1
        self.atrous2 = Conv(c_, c_, 3, 1, p=2, d=2)  # Dilated conv with dilation rate 2
        self.atrous3 = Conv(c_, c_, 3, 1, p=4, d=4)  # Dilated conv with dilation rate 4
        self.conv2 = Conv(c_ * 3, c2, 1, 1)  # Final 1x1 convolution to reduce channels

    def forward(self, x):
        x = self.conv1(x)  # Initial 1x1 convolution
        x1 = self.atrous1(x)  # Parallel dilated convolutions
        x2 = self.atrous2(x)
        x3 = self.atrous3(x)
        return self.conv2(
                    ...
                    if is_input:
                        if -1 in tuple(model.get_tensor_shape(name)):  # dynamic
                            ...
                            context.set_input_shape(
                                name, tuple(model.get_profile_shape(name, 0)[2])
                            )
                        if dtype == np.float16:
                            fp16 = True
                    else:  # output
