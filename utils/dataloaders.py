...
class LoadScreenshots:
    ...
    def __next__(self):
        ...
        s = (
            f"screen {self.screen} (LTWH): {self.left},{self.top},"
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(
        ...
        assert nf > 0 or not augment, f"{prefix}No labels found in {cache_path}, cannot start training."

        [cache.pop(k) for k in ("hash", "version", "msgs")]
        labels, shapes, self.segments = zip(*cache.values())
        nl = len(np.concatenate(labels, 0))
        assert nl > 0 or not augment, f"{prefix}All labels empty in dataset"
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if labels.size:
                labels[:, 1:] = xywhn2xyxy(labels[:, 1:], ratio[0] * w, ratio[1] * h, pad[0], pad[1])
    ...
    def load_image(self, index):
        """Loads image from file and resizes."""
        ...
        if r != 1:
            interp = random.choice([cv2.INTER_LINEAR, cv2.INTER_AREA])
    ...
    def check_cache_ram(self, safety_margin=0.1, prefix=""):
        ...
        if not cache:
            LOGGER.info(f"{prefix}{mem_required / gb:.1f}GB RAM required, {mem.available / gb:.1f}/{mem.total / gb:.1f}GB available")
        return cache
...
def verify_image_label(args):
    ...
    try:
        ...
        if im.format.lower() in ("jpg", "jpeg"):
            with open(im_file, "rb") as f:
                ...
                if f.read() != b"\xff\xd9":  # corrupt JPEG
                    ...
                    msg = f"{prefix}WARNING ⚠️ {
                        im_file}: corrupt JPEG restored and saved"
        ...
        if os.path.isfile(lb_file):
            ...
            if nl := len(lb):
                ...
                if len(i) < nl:  # duplicate row check
                    ...
                    msg = f"{prefix}WARNING ⚠️ {im_file}: {
                        nl - len(i)} duplicate labels removed"
            else:
                ne = 1  # label empty
        ...
        nc = 1
        msg = f"{prefix}WARNING ⚠️ {
            im_file}: ignoring corrupt image/label: {e}"
...
class HUBDatasetStats:
    ...
    def _find_yaml(dir):
        ...
        if len(files) > 1:
            ...
            assert (
                files
            ), f"Multiple *.yaml files found in {
                dir}, only 1 *.yaml file allowed"
        assert (
            len(files) == 1
        ), f"Multiple *.yaml files found: {
            files}, only 1 *.yaml file allowed in {dir}"
        return files[0]
    ...
    def _unzip(self, path):
        ...
        assert (
            ...
            dir} not found. path/to/abc.zip MUST unzip to path/to/abc/"
...
class ClassificationDataset(torchvision.datasets.ImageFolder):
    ...
    def __getitem__(self, i):
        ...
        return sample, j  # Ensure this is inside the function
